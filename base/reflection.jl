...
function fieldname(t::DataType, i::Integer)
    ...
    getfield(t, :abstract) && throw_not_def_field()
    names = _fieldnames(t)
...
"""
ismutable(@nospecialize(x)) = (@_pure_meta; getfield(typeof(x), :mutable))
...
function ismutabletype(@nospecialize(t::Type))
    ...
    return isa(t, DataType) && getfield(t, :mutable)
end
...
function isstructtype(@nospecialize(t::Type))
    ...
    return hasfield || (getfield(t, :size) == 0 && !getfield(t, :abstract))
end
...
function isabstracttype(@nospecialize(t))
    ...
    t = unwrap_unionall(t)
    # TODO: what to do for `Union`?
    return isa(t, DataType) && getfield(t, :abstract)
end
...
function fieldcount(@nospecialize t)
    ...
    if getfield(t, :name) === NamedTuple_typename
        ...
        abstr = getfield(t, :abstract) || (getfield(t, :name) === getfield(Tuple, :name) && isvatuple(t))
    end
    if abstr
        throw(ArgumentError("type does not have a definite number of fields"))
    end
